...
# Main grafana installation function
...
EOF
	...
	LOCAL_DASHBOARD="$SCRIPT_DIR/../dashboards/znnd.json"
	if [ -f "$LOCAL_DASHBOARD" ]; then
		echo "Using local dashboard file..."
		cp "$LOCAL_DASHBOARD" /tmp/znnd_dashboard.json
	else
		# Fallback to downloading from repository
		...
		echo "Downloading dashboard from branch ${GITHUB_BRANCH} of repo ${GITHUB_REPO}..."
		if ! curl -sf "$ZNND_DASHBOARD_JSON_URL" -o /tmp/znnd_dashboard.json; then
			echo "Error: Failed to download dashboard. Please ensure the file exists in your repository."
			exit 1
		fi
	fi
